...
def mkHand(name, length):
	# Register the Turtle shape, create a watch needle Turtle
    turtle.reset()
    Skip(-length * 0.1)
...
def Init():
    ...
    turtle.mode("logo")
    # Create three watch hands Turtle and initialize
    mkHand("secHand", 135)
    mkHand("minHand", 125)
    mkHand("hurHand", 90)
...
def Tick():
    # Draw the dynamic display of the dial
    t = datetime.today()
...
def main():
	# Turn on/off the turtle animation and set a delay for updating the drawing.
    turtle.tracer(False)
    Init()
    SetupClock(160)
    turtle.tracer(True)
